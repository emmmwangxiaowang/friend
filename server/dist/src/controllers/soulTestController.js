...
export async function getSoulTests(req, res) {
    try {
        const tests = await listSoulTests();
        // Return minimal payload to avoid over-fetching
        const payload = tests.map((t) => ({ id: t.id, title: t.title, description: t.description }));
        res.json(payload);
    }
    catch (err) {
        console.error(err);
...
export async function getSoulTest(req, res) {
...
export async function submitSoulTest(req, res) {
    const id = Number(req.params.id);
    const user = req.user;
...
export async function getSoulTestUserResult(req, res) {
    const id = Number(req.params.id);
    const user = req.user;
    const userId = user?.id;
    if (!id || !userId)
...
export async function getSoulTestMatches(req, res) {
    const id = Number(req.params.id);
    const user = req.user;
    const userId = user?.id;
    if (!id || !userId)
