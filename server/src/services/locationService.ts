...
const privacyMap: Map<number, boolean> = new Map();
...
function inferRegionFromCity(city: string): string | undefined {
  return CITY_REGION_MAP[city] ?? undefined;
}

export async function updateLocation(payload: LocationPayload): Promise<void> {
...
export async function getNearby(userId: number, radiusKm: number = 50): Promise<NearbyResult[]> {
...
export async function filterUsersByCity(city: string): Promise<NearbyResult[]> {
  const profiles = await prisma.profile.findMany({
    where: {
      location: {
        isNot: null,
  ...
  for (const p of profiles) {
    ...
    if (loc.city?.toLowerCase() === city.toLowerCase()) {
      if (privacyMap.get(p.userId) === true) continue;
      results.push({
        userId: p.userId,
        distanceKm: 0,
        city: loc.city,
...
export async function discoverSameCityOnly(userId: number, sameCity: boolean): Promise<NearbyResult[]> {
  if (!sameCity) {
