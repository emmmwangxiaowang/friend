...
export async function listSoulTests(): Promise<any[]> {
...
export async function getSoulTestWithQuestions(testId: number): Promise<any | null> {
...
export async function submitSoulTestAnswers(userId: number, soulTestId: number, answers: any): Promise<any> {
...
export async function getSoulTestResult(userId: number, soulTestId: number): Promise<any | null> {
...
export async function computeMatchesForUser(userId: number, soulTestId: number): Promise<Array<any>> {
  ...
  function perQuestionSimilarity(
    q: any,
    a: any,
    b: any,
  ): number {
  ...
  function categoryOfIndex(index: number): 'personality' | 'interests' | 'values' {
    if (index < counts.personality) return 'personality';
    if (index < counts.personality + counts.interests) return 'interests';
    return 'values';
  }
  ...
  const ansBMap = new Map<number, any>();
  ...
  for (const r of results) {
    ...
    questions.forEach((qq: any, idx: number) => {
      ...
      if (cat === 'personality') personalitySum += s; else if (cat === 'interests') interestsSum += s; else valuesSum += s;
    });
