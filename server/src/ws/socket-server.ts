  ...
  io.on('connection', async (socket) => {
    ...
    socket.on('message', async (payload) => {
      ...
      if (!conversationId && toUserId) {
        const conv = await prisma.conversation.create({ data: { type: 'PRIVATE' } as any });
        await prisma.userConversation.create({ data: { conversationId: conv.id, userId } as any });
        await prisma.userConversation.create({ data: { conversationId: conv.id, userId: toUserId } as any });
    ...
    socket.on('groupMessage', async (payload) => {
      ...
      const content = payload?.content;
      const conv = await prisma.conversation.create({ data: { type: 'GROUP', group: { connect: { id: groupId } } } as any });
    ...
    socket.on('typing', (payload) => {
      ...
      if (conversationId) io.to(`conversation_${conversationId}`).emit('typing', { userId, isTyping });
    });

    // Read receipts
    socket.on('read', (payload) => {
      const conversationId = payload?.conversationId;
      if (conversationId) io.to(`conversation_${conversationId}`).emit('read', { userId, conversationId });
    });
