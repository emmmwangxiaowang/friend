...
export default function SoulTestTakePage({ params }: { params: { id: string } }) {
  ...
  const handleAnswer = (option: string) => {
    ...
    if (currentQuestion < testQuestions.length - 1) {
      ...
      setResult(testResults[resultKey]);
      setShowResult(true);
    }
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 max-w-md w-full">
        <div className="mb-6">
          <div className="flex justify-between text-sm text-gray-500 mb-2">
            <span>Soul测试</span>
            <span>{currentQuestion + 1} / {testQuestions.length}</span>
          </div>
        ...
        <h2 className="text-xl font-bold mb-6">{testQuestions[currentQuestion].text}</h2>
        ...
        <div className="mt-6 text-center">
          <Link href="/soul-test" className="text-purple-600 dark:text-purple-400 hover:underline">
            返回测试列表
          </Link>
        </div>
      </div>
